...
def refresh_db():
    ...
    for i, element in enumerate(doc.xpath(XPATHS["grid"])):
        ...
        data["start_date"] = parse_date(
            details_box.xpath(XPATHS["start-date"] + XPATHS["date-suffix"])[0]
        )
        # TODO: Change this to something more reasonable
        try:
            data["end_date"] = parse_date(
                details_box.xpath(XPATHS["end-date"] + XPATHS["date-suffix"])[0]
            )
        except Exception:
            pass
        try:
            data["location"] = details_box.xpath(
                XPATHS["location"] + XPATHS["location-suffix"]
            )[0]
        except Exception:
            pass
        ...
        categories += [
            Category(name=i.strip(), event=event)
            for i in element.xpath(XPATHS["categories"])
        ]
        events.append(event)
